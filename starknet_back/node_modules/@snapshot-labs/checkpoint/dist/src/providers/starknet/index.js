...
class StarknetProvider extends base_1.BaseProvider {
    provider;
    processedPoolTransactions = new Set();
    ...
    async processBlock(blockNum) {
        ...
        try {
            ...
            if (!(0, types_1.isFullBlock)(block) || block.block_number !== blockNum) {
            ...
            if (e.message.includes('Block not found')) {
                ...
                throw new base_1.BlockNotFoundError();
            }
    ...
    async processPool(blockNumber) {
        const txs = await this.provider.getPendingTransactions();
        const receipts = await Promise.all(txs.map(async (tx) => {
            ...
            try {
                ...
                this.log.warn({ transactionHash: tx.transaction_hash, err }, 'getting transaction receipt failed');
                return null;
            }
        }));
    ...
    async handleBlock(block, eventsMap) {
        this.log.info({ blockNumber: block.block_number }, 'handling block');
        const txsToCheck = block.transactions.filter(tx => !this.processedPoolTransactions.has(tx.transaction_hash));
        for (const [i, tx] of txsToCheck.entries()) {
            await this.handleTx(block, block.block_number, i, tx, tx.transaction_hash ? eventsMap[tx.transaction_hash] || [] : []);
        }
        this.processedPoolTransactions.clear();
        this.log.debug({ blockNumber: block.block_number }, 'handling block done');
    }
    async handlePool(txs, eventsMap, blockNumber) {
        this.log.info('handling pool');
        const txsToCheck = txs.filter(tx => !this.processedPoolTransactions.has(tx.transaction_hash));
        for (const [i, tx] of txsToCheck.entries()) {
            await this.handleTx(null, blockNumber, i, tx, tx.transaction_hash ? eventsMap[tx.transaction_hash] || [] : []);
            this.processedPoolTransactions.add(tx.transaction_hash);
        }
        this.log.info('handling pool done');
    }
    async handleTx(block, blockNumber, txIndex, tx, events) {
        ...
        if (this.instance.config.global_events) {
            const globalEventHandlers = this.instance.config.global_events.reduce((handlers, event) => {
                handlers[`0x${starknet_1.hash.starknetKeccak(event.name).toString('hex')}`] = {
                    name: event.name,
                    fn: event.fn
                };
                return handlers;
            ...
            for (const [eventIndex, event] of events.entries()) {
                ...
                if (!handler)
                    continue;
                this.log.info({ contract: event.from_address, event: handler.name, handlerFn: handler.fn }, 'found contract event');
        ...
        for (const source of this.instance.config.sources || []) {
            ...
            const contract = (0, starknet_1.validateAndParseAddress)(source.contract);
            if ((0, types_1.isDeployTransaction)(tx) &&
                source.deploy_fn &&
                contract === (0, starknet_1.validateAndParseAddress)(tx.contract_address)) {
            ...
            for (const [eventIndex, event] of events.entries()) {
                if (contract === (0, starknet_1.validateAndParseAddress)(event.from_address)) {
                    for (const sourceEvent of source.events) {
                        if (`0x${starknet_1.hash.starknetKeccak(sourceEvent.name).toString('hex')}` === event.keys[0]) {
                            foundContractData = true;
                            ...
                            if (source.abi && this.abis?.[source.abi]) {
                                try {
                                    parsedEvent = (0, utils_1.parseEvent)(this.abis?.[source.abi], sourceEvent.name, event.data);
                                }
                                catch (err) {
                                    this.log.warn({ contract: source.contract, txType: tx.type, handlerFn: source.deploy_fn }, 'failed to parse event');
                                }
                            }
